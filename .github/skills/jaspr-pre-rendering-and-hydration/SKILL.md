...
When pre-rendering in `server` or `static` mode, you often need to fetch data asynchronously (e.g., from a database or API) _before_ rendering the HTML. Jaspr provides special server-only components to delay the build phase until the data is ready.
