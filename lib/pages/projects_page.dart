  ...
  Component build(BuildContext context) {
    if (projects.isEmpty) {
      return div(classes: 'min-h-screen flex items-center justify-center', [
        div(classes: 'text-center py-16', [.text('No projects found')]),
      ]);
    }

    return Component.fragment([
      div(classes: 'min-h-screen', [
        div(classes: 'container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl', [
          // Header Section
          div(classes: 'pt-24 pb-8', [_buildHeader()]),
        ...
        ProjectsView(projects: projects.toList()),

        // CTA Section
        div(classes: 'container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl', [
          div(classes: 'pt-8 pb-16', [_buildCTASection(context)]),
        ]),
      ]),
    ]);
  }
...
Component _buildFeaturedProjects(List<Project> featuredProjects) {
  return div(classes: 'mb-16', [
    h2(classes: 'text-3xl font-bold font-display mb-8', [
      span(classes: 'gradient-text', [.text('Featured')]),
      .text(' Projects'),
    ]),
    div(classes: 'grid lg:grid-cols-2 gap-8', [
      for (int index = 0; index < featuredProjects.length; index++)
        FeaturedProjectCard(project: featuredProjects[index], index: index),
    ]),
  ]);
}

Component _buildCTASection(BuildContext context) {
  return div(classes: 'text-center mt-8', [
    Card(
      ...
      children: [
        CardContent(
          ...
          children: [
            h3(classes: 'text-2xl font-bold font-display mb-4', [
              .text('Have a Project in '),
              span(classes: 'gradient-text', [.text('Mind?')]),
            ]),
            p(classes: 'text-muted-foreground mb-6 max-w-2xl mx-auto', [
              .text(
