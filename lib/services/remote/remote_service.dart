...
// Simulated RemoteService with Local 
import 'dart:math';
import 'package:portfolio_site/data/achievements.dart';
import 'package:portfolio_site/data/articles.dart';
import 'package:portfolio_site/data/projects.dart';
import 'package:portfolio_site/models/about_model.dart';
import 'package:portfolio_site/models/article_model.dart';
import 'package:portfolio_site/models/project_model.dart';
...
class RemoteService {
  Future<List<Project>> getProjects() async {
    // Simulate network delay (random between 700ms to 1500ms)
    await Future.delayed(Duration(milliseconds: 700 + Random().nextInt(800)));
    return projectsData.map((Project.fromJson)).toList();
  }

  Future<List<Article>> getArticles() async {
    // Simulate network delay (random between 700ms to 1500ms)
    await Future.delayed(Duration(milliseconds: 700 + Random().nextInt(800)));
    return articlesData.map((Article.fromJson)).toList();
  }

  Future<List<Achievement>> getAchievements() async {
    // Simulate network delay (random between 700ms to 1500ms)
    await Future.delayed(Duration(milliseconds: 700 + Random().nextInt(800)));
    return timelineData.map((Achievement.fromJson)).toList();
  }
}
