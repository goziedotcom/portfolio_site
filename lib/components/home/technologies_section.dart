...
class TechnologiesSection extends StatelessComponent {
  ...
  Component _buildSectionHeader() {
    return div(classes: 'text-center mb-16', [
      h2(classes: 'text-3xl md:text-4xl font-bold font-display mb-4', [
        .text('Technologies & '),
        span(classes: 'gradient-text', [.text('Skills')]),
      ]),
      p(classes: 'text-xl text-muted-foreground max-w-2xl mx-auto', [
        .text(
          'Modern tools and technologies I use to build cutting-edge digital experiences',
        ),
      ]),
    ]);
  }
  ...
  Component _buildTechnologiesGrid() {
    return div(classes: 'grid md:grid-cols-2 lg:grid-cols-3 gap-8', [
      for (
        int categoryIndex = 0;
        categoryIndex < SiteConfig.technologiesData.length;
        categoryIndex++
      )
        _buildTechnologyCategory(
          SiteConfig.technologiesData[categoryIndex],
          categoryIndex,
        ),
    ]);
  }

  Component _buildTechnologyCategory(
    ...
    return div(
      ...
      [
        // Category title
        h3(classes: 'text-lg font-semibold font-display text-primary', [
          .text(category.name),
        ]),
        ...
        div(classes: 'flex flex-wrap gap-2', [
          for (
            int techIndex = 0;
            techIndex < category.technologies.length;
            techIndex++
          )
  ...
  Component _buildTechnologyBadge(
    String technology,
    int categoryIndex,
    int techIndex,
  ) {
  ...
  Component _buildExperienceHighlight() {
    return div(classes: 'mt-16 text-center', [
      div(
        classes:
        ...
        attributes: {'aria-label': 'Experience highlight'},
        [
          div(classes: 'text-center text-primary-foreground', [
            div(classes: 'text-2xl font-bold mb-2', [.text('${SiteConfig.yearsOfExperience}+ Years')]),
            div(classes: 'text-primary-foreground/90', [
